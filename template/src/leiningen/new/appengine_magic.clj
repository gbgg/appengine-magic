(ns leiningen.new.appengine-magic
...
(defn appengine-magic
  "A Leiningen template for a new appengine-magic project"
  [projname & args]
  ;; projname syntax:  <clj-app>:<gae-app-id>
  (do
    (if (nil? args)
      (do (abort "missing sdk path!  Syntax:  lein new appengine-magic myproj path/to/sdk") )
      (println args))
    (let
        [[appname appid] (.split  projname ":")
         sdk (first args)
         render (renderer "appengine-magic")
      ...
      (println "Generating an appengine-magic project called " appname ", app id " appid ", using the 'appengine-magic' template.")
